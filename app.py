...
def univariate_likelihood(theta, model, observations, likelihood_method):
  ...
  for i in range(0, n):
    xt = (observations[i, 1, model.n] + model.G_0(theta)) / model.G(theta)
    yield_errors.append(-model.G_0_ext(theta) + model.G_ext(theta) * xt - observations[i, model.n + 1, 2 * model.n])

  error_distribution = norm(0, np.std(yield_errors))
  ...
  for i in range(1, n):
    observations_likelihood += model.l_g(likelihood_method, delta, observations[i, 1, model.n], observations[i - 1, 1, model.n], theta)
...
def multivariate_likelihood(theta, model, observations, likelihood_method):
  ...
  for i in range(0, observations.length()):
    xt = inv(model.G(theta).transpose()).dot(subtract_vectors(observations[i, 1, 2], model.G0(theta)))
    yield_errors.append(subtract(model.G0_ext(theta) + model.G_ext(theta).dot(xt), observations[i, 3, 4]))

  error_distribution = multivariate_normal(mean = np.zeros(2), cov = np.diag(multi_std(yield_errors)))
